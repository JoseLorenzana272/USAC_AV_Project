...
void mostrar_uso(const char *nombre_programa) {
    fprintf(stderr, "Usage: %s -p <input_file_route> -o <output_file_route> -j <num_threads> -k <key_file_route>\n", nombre_programa);
}
...
int generar_clave(const char *ruta_clave, size_t tamano) {
    ...
    printf("Key file %s generated succesfully (%zu bytes)\n", ruta_clave, tamano);
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    if (access(archivo_clave, F_OK) == -1) {
        printf("The key file %s does not exist. Generating one of 256 bytes...\n", archivo_clave);
        if (generar_clave(archivo_clave, 256) != 0) {
            return EXIT_FAILURE;
        }
    }

    long resultado = syscall(sys_xor_encrypt, archivo_entrada, archivo_salida, archivo_clave, num_hilos);
    if (resultado < 0) {
        perror("Error in the call of xor_encrypt");
        return EXIT_FAILURE;
    }
